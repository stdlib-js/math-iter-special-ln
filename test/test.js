...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterLn, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterLn( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the natural logarithm for each iterated value', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterLn( linspace( 0.0, 100.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( 0.0, 100.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = ln( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the natural logarithm for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
